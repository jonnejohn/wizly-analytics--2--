...
@Injectable()
export class OrganizationUnitObjectService {
    ...
    getCustomAttributes(username,password,modelid,webservice, hostUrl ): Promise<OrganizationUnitObject[]> {
        debugger;
        ...
        .toPromise()
        .then(response => response.json() as OrganizationUnitObject[])
        .catch(this.handleError);
        } 

        //api/QPRQueryObjectXML
    
    postOrganizationUnitObject(queryObject: OrganizationUnitObject,url:string): Observable<any> {
        debugger;
